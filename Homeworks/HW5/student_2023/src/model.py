...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.C = nn.init.xavier_uniform_(self.C)
        ### END YOUR CODE
    ...
    def forward(self, x_input):
        ...
        cross_attention = self.attn(self.ln1(x_input), self.C)
        res = self.C + cross_attention
        res = res + self.mlp(self.ln2(res))
        return res
        ### END YOUR CODE
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        ### Hint: Copy over the code from Block and make necessary modifications.
        ### Should be around 3-5 lines.
        cross_attention = self.attn(self.ln1(y), x_input)
        res = x_input + cross_attention
        res = res + self.mlp(self.ln2(res))
        return res
        ### END YOUR CODE
