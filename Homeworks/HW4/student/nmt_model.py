...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        permuteX = torch.permute(X, (1, 2, 0))
        embedX = self.post_embed_cnn(permuteX)
        permuteX = torch.permute(embedX, (2, 0, 1))
        paddX = pack_padded_sequence(permuteX, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(paddX)
        pad_enc_hiddens,_ = pad_packed_sequence(enc_hiddens)
        enc_hiddens = torch.permute(pad_enc_hiddens, (1, 0, 2))
        init_decode = torch.cat((last_hidden[0], last_hidden[1]), 1)
        init_decode = self.h_projection(init_decode)
        init_decoder_cell = torch.cat((last_cell[0], last_cell[1]), 1)
        init_decoder_cell = self.c_projection(init_decoder_cell)
        dec_init_state = (init_decode, init_decoder_cell)
        ### END YOUR CODE
