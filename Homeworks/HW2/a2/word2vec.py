...
def naiveSoftmaxLossAndGradient(
    ...
    y_pred[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, y_pred)
    gradOutsideVecs = np.dot(y_pred.reshape(-1, 1), centerWordVec.reshape(1, -1))
...
def negSamplingLossAndGradient(
    ...
    y_pred = sigmoid(np.dot(u_o, v_c))
    loss = -1 * np.log(y_pred)
    gradCenterVec = np.dot(u_o, y_pred - 1)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = np.dot(v_c, y_pred - 1)
    for i in range(K):
        idx = indices[i + 1]
        u_k_o = outsideVectors[idx]
        y_k_pred = sigmoid(-1 * np.dot(u_k_o, v_c))
        loss -= np.log(y_k_pred)
        y_prime = 1.0 - y_k_pred
        gradOutsideVecs[idx] += np.dot(y_prime, v_c)
        gradCenterVec += np.dot(y_prime, u_k_o)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        wordLoss, gradCenter, gradOutside = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += wordLoss
