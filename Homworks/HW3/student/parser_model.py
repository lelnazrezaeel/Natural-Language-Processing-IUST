...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        emb_hidden = self.embed_to_hidden_weight
        x = torch.matmul(emb, emb_hidden)
        emb_bias_hidden = self.embed_to_hidden_bias
        h = F.relu(x + emb_bias_hidden)
        hidden_logits_weight = self.hidden_to_logits_weight
        h = torch.matmul(h, hidden_logits_weight)
        hidden_logits_bias = self.hidden_to_logits_bias
        logits = h + hidden_logits_bias
        ### END YOUR CODE
        return logits
