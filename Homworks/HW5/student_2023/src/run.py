...
if args.variant == 'vanilla':
    ...
    device = "cuda:0"
    model_fine = model.GPTConfig(pretrain_dataset.vocab_size,
                            pretrain_dataset.block_size,
                            n_layer=4, n_head=8, n_embd=256,
                            perceiver=True,
                            bottleneck_dim=args.bottleneck_dim)
    model = model.GPT(model_fine)
...
if args.function == 'pretrain':
    ...
    model_fine = trainer.TrainerConfig(max_epochs=650, 
                                       batch_size=128,
                                       learning_rate=args.pretrain_lr,
                                       lr_decay=True, 
                                       warmup_tokens=512*20,
    ...
    model = model.to(device)
    trainer_ = trainer.Trainer(model, pretrain_dataset, None, model_fine).train()
    ...
    if args.reading_params_path is None: 
        model_fine = trainer.TrainerConfig(max_epochs=75, 
                                           batch_size=256, 
                                           learning_rate=args.finetune_lr,
                                           lr_decay=True, 
                                           warmup_tokens=512*20,
        ...
        model_fine = trainer.TrainerConfig(max_epochs=10, 
                                      batch_size=256, 
                                      learning_rate=args.finetune_lr,
    ...
    txt_fine = open(args.finetune_corpus_path).read()
    ds_fine = dataset.NameDataset(pretrain_dataset, txt_fine)
    model = model.to(device)
    trainer_ = trainer.Trainer(model, ds_fine, None, model_fine).train()
    torch.save(model.state_dict(), args.writing_params_path)
