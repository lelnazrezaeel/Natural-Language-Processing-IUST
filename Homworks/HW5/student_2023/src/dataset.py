...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        stop_point = self.block_size * (7/8) + 1
        rnd = random.uniform(4, int(stop_point))
        truncated_length = int(rnd)
        document = document[:truncated_length]
        document_length = len(document)
        location = (1/4) * document_length
        scale = (1/4) * document_length / 5
        masked_length = max(1, int(np.random.normal(location, scale)))
        stop_point = random.uniform(0, document_length - masked_length - 1)
        start_index = int(stop_point)
        end_index = start_index + masked_length
        prefix = document[0:start_index]
        masked_content = document[start_index:end_index]
        suffix = document[end_index:]
